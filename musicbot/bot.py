...
class MusicBot(discord.Client):
    ...
    async def cmd_download(self, author):
        """
        Usage:
    ...
    async def cmd_toggle(self, message, user_mentions, option, something):
        """
        Usage:
        ...
        old_len = len(self.toggle)

        if option in ['+', 'add']:
            self.toggle.append(user_mentions[0])

            return Response(
                '%s users have been added to the hitlist' % (len(self.toggle) - old_len),
                reply=True, delete_after=10
            )

        else:
            if user_mentions[0] in self.toggle:
                self.toggle.remove(user_mentions[0])

                return Response(
                    '%s users have been removed from the hitlist' % (old_len - len(self.toggle)),
                    reply=True, delete_after=10
                )
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if author in self.toggle:
            await self.cmd_futari(player, channel, author, permissions, leftover_args)
            return
    ...
    async def cmd_futari(self, player, channel, author, permissions, leftover_args):
        """
        Usage:
            {command_prefix}futari

        Adds the futari to the playlist.  If a link is not provided, the first
    ...
    async def cmd_brian(self, player, channel, author, permissions, leftover_args):
        """
        Usage:
    ...
    async def cmd_remove(self, player, author, index):
        """
        Usage:
        ...
        holder = player.playlist.remove(int(index) - 1)
        return Response('Deleted `{}.` **{}** added by **{}**'.format(index, holder.title, holder.meta['author'].name).strip(), delete_after=20)


    async def cmd_bump(self, player, author, index):
        """
        Usage:
            {command_prefix}bump n

        Bumps the nth song in queue to the front of the queue
        """
        if int(index) > len(player.playlist.entries):
            return Response('Invalid number.', delete_after=20)
        ...
        holder = player.playlist.remove(int(index) - 1)
        player.playlist.addstart(holder)
        return Response('Bumped`{}.` **{}** added by **{}**'.format(index, holder.title, holder.meta['author'].name).strip(), delete_after=20)




    async def cmd_noskip(self, player, author):
        """
        Usage:
            {command_prefix}noskip

        Turns on noskip mode: only owner may skip a song.
        """
        if self.noskip and author.id == self.config.owner_id:
            self.noskip = False
            return Response('Bot is no longer in noskip mode!', reply=True, delete_after=20)
        if self.noskip and author.id != self.config.owner_id:
            return Response('Only the onwer can turn off noskip mode!', reply=True, delete_after=20)
        else:
            self.noskip = True
            return Response('Bot is now in noskip mode!', reply=True, delete_after=20)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        """
        if self.noskip and author.id != self.config.owner_id:
            raise exceptions.CommandError("Cannot skip. Bot is currently in noskip mode!", expire_in=20)
